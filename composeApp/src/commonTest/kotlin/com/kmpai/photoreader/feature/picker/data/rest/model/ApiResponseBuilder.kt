...
class ApiResponseBuilder {
    ...
    fun build(): APIResponse {
        return APIResponse(id, choices, created, model, imageResponseObject, systemFingerprint, usage)
    }
}
class ChoiceBuilder {
    private var finishReason: String? = "finishReasonBuilder"
    private var index: Long? = 0L
    private var message: APIMessage? = ApiMessageBuilder().build()

    fun finishReason(finishReason: String) = apply { this.finishReason = finishReason }
    fun index(index: Long) = apply { this.index = index }
    fun message(message: APIMessage) = apply { this.message = message }
